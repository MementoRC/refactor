...
@_hint("deprecated_alias", "NewStatementAction")
@dataclass
class LazyInsertAfterWithSeparator(_LazyActionMixin[ast.stmt, ast.stmt]):
    """Inserts the re-synthesized version :py:meth:`LazyInsertAfter.build`'s
    output right after the given `node`.
...
@_hint("deprecated_alias", "NewStatementAction")
@dataclass
class LazyInsertBeforeWithSeparator(_LazyActionMixin[ast.stmt, ast.stmt]):
    """Inserts the re-synthesized version :py:meth:`LazyInsertBefore.build`'s
    output right before the given `node`.
...
@_hint("deprecated_alias", "TargetedNewStatementAction")
@dataclass
class InsertAfterWithSeparator(LazyInsertAfterWithSeparator):
    """Inserts the re-synthesized version of given `target` right after
    the given `node`.
...
@_hint("deprecated_alias", "TargetedNewStatementBeforeAction")
@dataclass
class InsertBeforeWithSeparator(LazyInsertBeforeWithSeparator):
    """Inserts the re-synthesized version of given `target` right after
    the given `node`.
...
class Erase(_ReplaceCodeSegmentAction):
    ...
    def is_critical_node(self, context: Context) -> bool:
        ...
        return parent_field != "decorator_list" and isinstance(parent_field_value, list) and len(parent_field_value) == 1
